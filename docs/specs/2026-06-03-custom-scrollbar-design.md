...
- **DD-4 审美：macOS 细 overlay**。沿用原型语汇——thumb `w-1.5 rounded-full bg-foreground/35`，浮在右缘**不占布局**（不给 Content 留 gutter）。**显示时机 = 滚动时 ∪ 指针在滚动条轨道上时**：`data-scrolling` 时 `opacity-100` 瞬现；**对滚动条元素本身用 CSS `hover:opacity-100`**（指针移到右缘那条轨道才显示，便于抓拽 thumb）；停手/移开后经 `duration-300` 渐隐。**关键区分**：用的是滚动条**元素自身**的 `:hover`，**不是** Base UI 的 `data-hovering`——后者是「指针在整个滚动区内」，经实机评审太急、不符 macOS「show on scroll」预期，故弃用。（滚动条隐藏态仍 `pointer-events:auto`，Base UI 未内部置 none，故 `:hover` 可触发。）
...
- **`ScrollBar`**：`Scrollbar(orientation="vertical") > Thumb`。
  - Scrollbar：`flex w-2.5 touch-none select-none justify-center opacity-0 transition-opacity duration-300 hover:opacity-100 data-[scrolling]:opacity-100 data-[scrolling]:duration-0`（`data-scrolling` 瞬现、停手 `duration-300` 渐隐；`hover:`（条元素自身）指针移到轨道时显示。**不用整区 `data-[hovering]`**）。
...
- **手测清单**（`pnpm start`，逐容器）：
  ...
  - 7 处外壳容器，逐处：原生条消失 ✓ / 内容溢出时 thumb 现、不溢出时无 thumb ✓ / **滚动时淡入、停手淡出；指针移到右缘轨道时也显示，但悬停内容区不显** ✓ / thumb 可拖拽滚动且拖时不选中文本 ✓ / 暗色下 thumb 对比可见 ✓ / 无布局回归（高度/flex/padding 正确）✓
  - AIPanel：发新消息仍平滑自动滚底 ✓
...
- **overlay thumb 遮内容**：浮条压在内容极右缘——thumb 细、仅滚动或 hover 轨道时可见，可接受；个别容器按需补 `pe-*`。（注：滚动条 `pointer-events:auto` 会占右缘约 10px 条带的命中区，但各容器有内边距、极右缘无交互元素，实测无碍。）
...
- **DD-4**：macOS 细 overlay 审美（`w-1.5 rounded-full bg-foreground/35`，浮右缘不占布局，**滚动时 ∪ hover 滚动条轨道时显示、停手/移开渐隐**；用条元素自身 `:hover`，不认整区 `data-hovering`）。
