...
- 工具返回的 annotation、conversation、message、note、session ID，以及 UUID、序号、游标、memory slug 和工具名，只是 agent 导航与继续读取所需的内部元数据，绝不能出现在面向读者的报告中。需要表达依据时，应改用短引文、会话主题、章节、页码或日期等人类可读的上下文；没有可读上下文时直接自然叙述，不虚构引用标记。
...
- 默认提示词明确禁止把工具内部 ID、序号、游标、memory slug 或工具名写入报告，并指导 agent 用短引文、主题、章节、页码或日期表达可读依据。
