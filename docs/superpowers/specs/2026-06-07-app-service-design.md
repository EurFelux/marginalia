...
日期：2026-06-07（v2 修订：废除 `env` getter——`dataDir` 完全私有，公共面改为 `getPath(key)` + `isDev` + `openFolder`）
...
数据目录的暴露方式（v2）：**不暴露原始 `dataDir`**——若消费方各自拿根目录拼路径，目录布局的知识就散落在所有消费方里（拼错、漂移无从约束）。改为 `getPath(key)` 以**类型化 key** 按 module 发放专有路径（`DataPathKey`——dir/file 语义编码在 key 名里），布局与文件名知识收归 AppService 一处。
...
| API 面             | **按需设计**——公共面三项：`getPath(key)`（LoggerService 等需要专有路径）、`isDev`（dev 双写判断）、`openFolder(dir)`（`app:openLogsDir` handler 需要，`main.ts` 注入 `shell.openPath` 实现）。注入物仍含 `dataDir`，但它是 `#env` 私有字段，**永不暴露**                                                                                                                                                                                                                                                                                                                                                                                                              |
| 路径 key 化        | `getPath(key: DataPathKey): string`，`DataPathKey = "logsDir" \| "booksDir" \| "dbFile"`（类型化 key，按需扩展）。**dir/file 语义编码在 key 名里**：`*Dir` 返回目录、`*File` 返回完整文件路径。映射是 AppService 内部策略（统一 `join(dataDir, rel)`：`logsDir`→`logs`、`booksDir`→`books`、`dbFile`→`marginalia.db` 根下文件——文件名知识也收归此处，历史布局零迁移）。**消费方迁移在本 spec 范围**：删除 `instance.ts` 的 `getBooksDir`（`ai/send-deps.ts` ×2、`ipc/library-handlers.ts` ×4 改用 `getPath("booksDir")`）；`instance.ts` 的 dbPath 改用 `getPath("dbFile")`（迁移后 instance.ts 不再 import electron）。getPath 纯计算不碰 fs（目录创建是消费方的事） |
...
消费方（LoggerService、handlers 等）── import { appService } from "../app" ──→ getPath(key) / isDev / openFolder（恒可用）
```
...
- `app-service.test.ts`：未注入即调用 `getPath`/`isDev`/`openFolder` 任一成员抛错（fail-fast；本测试文件用 resetModules+动态 import 构造未注入态）；注入后 `getPath("logsDir")` = `<dataDir>/logs`、`getPath("booksDir")` = `<dataDir>/books`、`getPath("dbFile")` = `<dataDir>/marginalia.db`、`isDev` 即注入值；`openFolder` fake 断言被调用与参数；重复注入 last-wins；**dataDir 不以成员身份暴露**（公共面无 `env`/`dataDir` 成员）；barrel 导出面仅 `appService`（封装不泄露）；`grep -rn "getBooksDir" src/` 零命中（迁移完成的回归断言，手动验证即可）。
