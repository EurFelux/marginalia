...
已有报告的“重新生成”必须先弹出确认框，明确成功后会替换当前报告。用户确认后，旧报告继续展示，主操作变为“停止重新生成”；初次生成时也提供“停止生成”。停止是正常用户动作：立即回到由持久化内容派生的 `ready` 或 `empty` 状态，不显示失败提示。重新生成成功后自动替换旧稿，不增加候选稿确认、撤销或版本历史。
