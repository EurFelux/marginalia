...
## §10 开放问题（已全部裁决，2026-06-10）

1. **Lia 的主动性**：**否决**——不做主动开场等机制，AI 保持被动响应。
2. **记忆的时间衰减/置信度**：**不做系统性机制**——记忆陈旧与否只有用户能判断，系统做不了有意义的事；维持「用户管理面板 + Lia 自主 `updateMemory`/`deleteMemory`」的现有维护通道。
3. **读完一本书的回顾仪式**：理念有价值，但**不属于记忆系统**——它是独立的仪式性环节，留待未来单独设计；届时顺手把回顾产出沉淀进记忆即可（记忆工具已就绪，零额外耦合）。
4. **记忆互链**：已于当日讨论后**纳入本期设计**，见 §2.1.1。
