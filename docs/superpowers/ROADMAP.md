...
| **数据备份与恢复**：导出/导入用户数据包，至少覆盖 SQLite DB、`userData/books/` 内的 ePub 副本、偏好与标注/进度/会话数据；恢复时需处理版本/迁移、现有数据覆盖确认、备份完整性校验，以及密钥/凭据是否可迁移的边界（`safeStorage` 加密内容可能绑定当前系统用户环境，需设计清楚）。个人自用场景下优先做“手动导出 zip + 手动恢复”即可，不必先做云同步或自动备份。                                                                                                                                                                                                                                                                                                                                                                                                                                          | 🔴   | 用户 2026-06-04 提醒             |
