...
**API key 明文落库已交付**（2026-06-04）：`providers.api_key_encrypted`（BLOB）→ `api_key`（TEXT）；`Encryptor` 与 `safeStorage` 依赖退役；`ProviderDto.keyMask` 由 `{hasKey,keyMask,canEncrypt}` 三布尔扁平态改为 `keyMask: string | null`（有则掩码、无则 null）。根因：safeStorage 按签名身份隔离，dev/打包身份不同无法互解，且钥匙串加密内容无法随 DB 备份恢复——与「备份必须能恢复 key」的硬约束根本矛盾。详见 `specs/2026-06-04-plaintext-api-key-storage-design.md`。
