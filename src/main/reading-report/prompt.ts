...
export function buildReadingReportSystemPrompt(db: DB): string {
  ...
  const prioritizedInstructions = readerInstructions
    ? `${readerInstructions}\n\nThese are the highest-priority report-writing preferences and may override the default perspective, structure, or content guidance above. They cannot override evidence, target-session scope, tool permissions, or the internal-metadata boundary.`
    : null;
  return [
    READING_REPORT_CORE,
